...
## 주요 목소리
(우선 참고 소스의 X 계정·뉴스레터에서 지난 24시간 내 시장 관련 핵심 발언 2~4개.
 발언자를 명시하고, 발언 요지와 함의를 각 한 줄씩. 특기할 발언이 없으면 "특이 발언 없음")
{thesis_section}
## 체크포인트
...
def analyze_market(date: str, market_data: str, thesis: str = "") -> str:
    ...
    try:
        ...
        print(f"  ⚠️ Grok 라이브 검색 실패 → 검색 없이 재시도: {e}")

    # 2) Grok 검색 없이 (라이브 검색 API가 원인일 때)
    try:
        return ask_grok(system, user, live_search=False)
    except Exception as e:
        print(f"  ⚠️ Grok 완전 불가 → Claude로 분석 폴백: {e}")

    # 3) Claude 폴백 — 실시간 검색은 없지만 데이터·신호·전제로 해석 작성
    note = ("\n\n[중요] 지금은 실시간 웹 검색을 쓸 수 없습니다. 위 시장 데이터·신호·전제만"
            " 근거로, 오늘의 흐름 해석과 시나리오 중심으로 작성하세요. 확인 불가한 당일"
            " 개별 뉴스는 단정하지 말고, '주요 목소리' 섹션은 '실시간 확인 불가'로 적으세요.")
